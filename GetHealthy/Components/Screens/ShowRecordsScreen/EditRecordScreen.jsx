...
export default function EditRecordScreen() {
  ...
  useEffect(() => {
    setName(param?.record?.illnessName)
    setStartDate(param?.record?.illnessStartDate)
    setLocation(param?.record?.illnessLocation)
    setStatus(param?.record?.illnessStatus)
    setDescription(param?.record?.illnessDescription)
  }, [])
  ...
  const handleSubmit = () => {
    ...
    GlobalAPI.editRecord(recordData).then(resp => {
      Toast.show({
          type: 'success',
          text1: 'Success',
          text2: 'record edited Successfully successfully!',
        });
        // clearForm()
        setLoading(false)
        navigation.goBack()
